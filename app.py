...
# ====== SLIDERS + RETRAIN PANEL ======
st.sidebar.markdown("---")
st.sidebar.header("🔁 Retrain models on filtered data (experimental)")

def get_feature_range_from_defaults(f):
    return DEFAULT_RANGES.get(f, (0.0, 100.0))

def build_sliders_for_features(features, clean_df):
    sliders = {}
    for f in features:
        mn, mx = get_feature_range_from_defaults(f)
        # if clean_df has values, tighten defaults
        if clean_df is not None and f in clean_df.columns:
            try:
                col = pd.to_numeric(clean_df[f], errors='coerce').dropna()
                if len(col) > 0:
                    mn = float(min(mn, col.min()))
                    mx = float(max(mx, col.max()))
            except Exception:
                pass
        # create slider (min,max) with step inferred
        step = max((mx - mn) / 200.0, 0.01)
        low, high = st.sidebar.slider(f"{f} range", min_value=float(mn), max_value=float(mx),
                                      value=(float((mn+mx)/2.0 - (mx-mn)/4.0), float((mn+mx)/2.0 + (mx-mn)/4.0)),
                                      step=step)
        sliders[f] = (float(low), float(high))
    return sliders

# show sliders for the union of features used by both models (or do separately)
clean_df = ARTS.get('clean_data')
all_features_for_sliders = list(dict.fromkeys((features_cls or []) + (features_reg or [])))
if len(all_features_for_sliders) > 0:
    st.sidebar.markdown("**Filter training data by ranges**")
    sliders = build_sliders_for_features(all_features_for_sliders, clean_df)
else:
    sliders = {}

st.sidebar.markdown("### Retrain controls")
retrain_do = st.sidebar.button("🔁 Retrain models on filtered data")

def filter_df_by_sliders(df, sliders):
    if df is None or len(sliders)==0:
        return df
    mask = pd.Series([True]*len(df), index=df.index)
    for f,(low,high) in sliders.items():
        if f in df.columns:
            try:
                col = pd.to_numeric(df[f], errors='coerce')
                mask = mask & col.between(low, high, inclusive="both")
            except Exception:
                pass
        else:
            # if column missing, drop rows (no match)
            mask = mask & False
    return df[mask].copy()

if retrain_do:
    if clean_df is None:
        st.error("No clean_data.csv found in artifacts — cannot retrain.")
    else:
        with st.spinner("Filtering data and retraining models..."):
            filtered = filter_df_by_sliders(clean_df, sliders)
            n_rows = 0 if filtered is None else len(filtered)
            st.info(f"Filtered rows: {n_rows}")
            if n_rows < 30:
                st.warning("Too few rows to retrain reliably (need >=30). Adjust sliders or upload more data.")
            else:
                # --- Classification retrain (if possible) ---
                if ARTS.get('clf') is not None and features_cls and 'Machine failure' in filtered.columns:
                    try:
                        Xc = filtered[features_cls].apply(pd.to_numeric, errors='coerce').fillna(0.0)
                        yc = filtered['Machine failure'].astype(int)
                        from sklearn.ensemble import RandomForestClassifier
                        from sklearn.model_selection import cross_val_score, KFold
                        clf_new = RandomForestClassifier(n_estimators=200, random_state=42, class_weight='balanced', n_jobs=-1)
                        cv = KFold(n_splits=10, shuffle=True, random_state=42)
                        scores_f1 = cross_val_score(clf_new, Xc, yc, cv=cv, scoring='f1', n_jobs=-1)
                        scores_acc = cross_val_score(clf_new, Xc, yc, cv=cv, scoring='accuracy', n_jobs=-1)
                        clf_new.fit(Xc, yc)
                        st.success(f"Classifier retrained: F1 (cv mean) = {scores_f1.mean():.3f} ± {scores_f1.std():.3f}; Acc (cv mean) = {scores_acc.mean():.3f}")
                        # persist classifier
                        joblib.dump(clf_new, ARTIFACTS_DIR / "model_failure_classifier_retrained.pkl")
                        st.write("Saved: artifacts/model_failure_classifier_retrained.pkl")
                    except Exception as e:
                        st.error("Error retraining classifier: " + str(e))
                else:
                    st.info("Skipping classifier retrain (classifier artifact missing or 'Machine failure' not in data).")

                # --- Regression retrain (if possible) ---
                if ARTS.get('reg') is not None and features_reg and 'Tool wear [min]' in filtered.columns:
                    try:
                        Xr = filtered[features_reg].apply(pd.to_numeric, errors='coerce').fillna(0.0)
                        yr = pd.to_numeric(filtered['Tool wear [min]'], errors='coerce')
                        mask = ~yr.isna()
                        if mask.sum() < 30:
                            st.warning("Not enough valid regression rows after filtering.")
                        else:
                            from sklearn.ensemble import RandomForestRegressor
                            from sklearn.model_selection import cross_val_score, KFold
                            reg_new = RandomForestRegressor(n_estimators=200, random_state=42, n_jobs=-1)
                            cv = KFold(n_splits=10, shuffle=True, random_state=42)
                            # cross_val_score returns neg_mean_squared_error for 'neg_mean_squared_error'
                            scores_rmse = cross_val_score(reg_new, Xr[mask], yr[mask], cv=cv, scoring='neg_mean_squared_error', n_jobs=-1)
                            # convert to RMSE
                            rmses = np.sqrt(-scores_rmse)
                            reg_new.fit(Xr[mask], yr[mask])
                            st.success(f"Regressor retrained: RMSE (cv mean) = {rmses.mean():.3f} ± {rmses.std():.3f} minutes")
                            # persist regressor
                            joblib.dump(reg_new, ARTIFACTS_DIR / "model_mtbf_randomforest_retrained.pkl")
                            st.write("Saved: artifacts/model_mtbf_randomforest_retrained.pkl")
                    except Exception as e:
                        st.error("Error retraining regressor: " + str(e))
                else:
                    st.info("Skipping regressor retrain (regressor artifact missing or 'Tool wear [min]' not in data).")
